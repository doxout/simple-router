...
function createMethodFilter(method, match) {
    ...
    return function filter(req) {
        ...
        var matches = match(req._parsedUrl.pathname);
        if (matches != null && matches.names !== null) {            
            if (!req.query) req.query = {};
            for (var k = 0; k < matches.names.length; ++k)
                req.query[matches.names[k]] = matches.values[k];
        }
        return matches;
    }
}
...
Router.prototype.setErrorHandler = function Router$setErrorHandler(errhandler) {
    if (typeof errhandler !== 'function' || errhandler.length != 4)
        throw new TypeError('Error handler must be function(err, req, res, next)');
    this._defaultErrorHandler = errhandler;
}
